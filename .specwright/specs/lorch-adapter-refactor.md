...
- [x] Tool adapter pattern documented and implemented
- [x] `lorch config show meltano` command displays meltano configuration
- [x] `lorch config sync meltano` syncs from meltano.yml to lorch cache
- [x] `lorch tools validate meltano` validates tap-target pairs before execution
- [x] Gmail extraction works correctly (1-day filter, text-only, ~1-5MB files)
- [x] Extract stage uses meltano adapter with validation
- [ ] **CanonizerAdapter** implemented with LATEST pointer support
- [ ] Canonize stage uses CanonizerAdapter
- [ ] **VectorProjectorAdapter** stub implemented
- [ ] Index stage uses VectorProjectorAdapter stub
...
### Step 10: Implement CanonizerAdapter [G1: Code Readiness]

**Prompt:**

Create `lorch/tools/canonizer.py` with `CanonizerAdapter` class.

Implement:
1. `load_config()` - Load from config/tools/canonizer.yaml
2. `sync_config()` - Discover transforms from registry, write to cache
3. `validate()` - Check canonizer installation, venv, transforms exist
4. `transform_from_manifest()` - Process vault manifest with LATEST pointers
5. `find_latest_manifests()` - Discover LATEST runs for source path

The adapter should:
- Support vault structure with manifests and gzip chunks
- Process LATEST runs only (deterministic, idempotent)
- Decompress gzip chunks and pipe to canonizer
- Clear output files before rebuild
- Return transform results with record counts

**Outputs:**

- `lorch/tools/canonizer.py`
- `config/tools/canonizer.yaml` (auto-generated by sync)

---

### Step 11: Update Canonize Stage to Use Adapter [G1: Code Readiness]

**Prompt:**

Update `lorch/stages/canonize.py` to use CanonizerAdapter:

1. Import CanonizerAdapter
2. Initialize adapter in __init__ or validate
3. Delegate manifest discovery to adapter
4. Delegate transform execution to adapter
5. Keep stage focused on orchestration (loop over mappings, logging, error handling)
6. Remove direct subprocess calls to canonizer
...
- `lorch/stages/canonize.py` (updated)

---

### Step 12: Implement VectorProjectorAdapter Stub [G1: Code Readiness]

**Prompt:**

Create `lorch/tools/vector_projector.py` with `VectorProjectorAdapter` **stub** class.

Since vector-projector tool isn't ready yet, implement minimal stub:
1. `load_config()` - Return empty config
2. `sync_config()` - No-op (nothing to sync yet)
3. `validate()` - Always return valid (stub mode)
4. `copy_files()` - Current stub behavior (copy canonical files)

Add TODO comments indicating:
- Future: SQLite indexing
- Future: Inode-style storage
- Future: Query API
- Current: Stub mode just copies files

**Outputs:**

- `lorch/tools/vector_projector.py`

---

### Step 13: Update Index Stage to Use Adapter Stub [G1: Code Readiness]

**Prompt:**

Update `lorch/stages/index.py` to use VectorProjectorAdapter:

1. Import VectorProjectorAdapter
2. Initialize adapter
3. Delegate file operations to adapter.copy_files()
4. Keep stage focused on orchestration
5. Add comments about future vector-projector integration
...
- `lorch/stages/index.py` (updated)

---

### Step 14: Test Complete Adapter Pattern [G2: Pre-Release]

**Prompt:**

Test all three adapters together:

1. Test CanonizerAdapter:
   - `lorch config sync canonizer`
   - `lorch tools validate canonizer`
   - `lorch run --stage canonize`
   - Verify LATEST pointers processed correctly
   - Verify canonical output files created

2. Test VectorProjectorAdapter stub:
   - `lorch run --stage index`
   - Verify files copied to vector-store

3. Test full pipeline:
   - `lorch run` (all stages)
...
```bash
cd /home/user/lorch
# Test canonizer adapter
lorch config sync canonizer
lorch tools validate canonizer
lorch run --stage canonize

# Test index adapter stub
lorch run --stage index
...
- ✓ Canonize stage processes LATEST manifests
- ✓ Index stage stub copies files
- ✓ No errors in pipeline execution

**Outputs:**

- Working adapter pattern across all stages
